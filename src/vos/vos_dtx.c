		   ...
		   bool *fatal)
{
	...
	if (dae != NULL) {
		...
		if (dth->dth_oid_cnt == 1) {
			...
			D_ALLOC_ARRAY(dce->dce_oids, dth->dth_oid_cnt);
			...
			memcpy(dce->dce_oids, dth->dth_oid_array,
			       sizeof(daos_unit_oid_t) * dth->dth_oid_cnt);
		}
	}
...
vos_dtx_prepared(struct dtx_handle *dth)
{
	...
	if (dth->dth_oid_array != NULL) {
		...
		if (dth->dth_oid_cnt == 1) {
			...
			D_ALLOC_ARRAY(dae->dae_oids, dth->dth_oid_cnt);
			if (dae->dae_oids == NULL) {
				...
				memcpy(dae->dae_oids, dth->dth_oid_array,
